...
class Pix2PixModel(nn.Module):
    ...
    # Entry point for all calls involving execute pass
    # of deep networks. We used this approach since DataParallel module
    # can't parallelize custom functions, we branch to different
    # routines based on |mode|.
    def execute(self, data, mode):
        input_semantics, real_image = self.preprocess_input(data)
        # print semantics, real_image
    ...
    def preprocess_input(self, data):
        ...
        if not self.opt.no_instance:
            ...
            input_semantics = jt.contrib.concat(
    ...
    def compute_discriminator_loss(self, input_semantics, real_image):
        ...
        _, pred_fake, _, pred_real = self.discriminate(
            input_semantics, fake_image, real_image)
    ...
    def discriminate(self, input_semantics, fake_image, real_image):
        fake_and_real_img = jt.contrib.concat([fake_image, real_image], dim=0)

        discriminator_out = self.netD(fake_and_real_img,
                                      segmap=jt.contrib.concat((input_semantics, input_semantics), dim=0))

        fake_feats, fake_preds, real_feats, real_preds = self.divide_pred(
            discriminator_out)
